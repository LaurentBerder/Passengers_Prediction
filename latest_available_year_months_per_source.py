...
def update_latest_available_dates():
    ...
    for a in soup.find_all('a'):
        if not a.get('href') or string not in a.get('href') or 'xls' not in a.get('href'):
            ...
            file = pd.ExcelFile(base_url + a.get('href'))
            if 'inter' in str(file.sheet_names).lower():
                provider_full = provider + ' - intl'
            else:
                provider_full = provider + ' - domestic'
            xl = file.parse(file.sheet_names[0], header=5)
