...
# -------------------------------------------------------------------------------
# Name:        Optimode / forecast_segment_with_Prophet
...
def main():
    ...
    model = Prophet(mcmc_samples=100, changepoint_prior_scale=0.001)
    ...
    model.plot(forecast).legend(handles=[forecast['t'], forecast['cap'], forecast['yhat']], labels=["Passengers", "Capacity", "Prediction"], loc=1)
    model.plot(forecast, uncertainty=True,  xlabel='Date', ylabel='Passengers')
    plt.title(origin + " - " + destination)
    model.plot(forecast, uncertainty=True,  xlabel='Date', ylabel='Passengers').savefig("/home/laurent/plot.jpg")
    plt.title(origin + " - " + destination)
    # The capacity doesn't look nice on the trend plot, so remove it before this plot
    del forecast['cap']
    model.plot_components(forecast, uncertainty=True, plot_cap=False)
    model.plot_components(forecast, uncertainty=True, plot_cap=False).savefig("/home/laurent/plot_components.jpg")
